...
def load_config(
    ...
    if experiment_name == "dental":
        ...
        model = pmfm.RegularisedPointManifoldFlowSTNModel.load_from_checkpoint(
            # model_path, n_components=n_components, fourier_feature_scale=0.4
            model_path,
            n_components=128,  # n_components,
    ...
    print("WARNING: estimating trace!!!")
    model.pdm.layers.ambient_flow.set_estimate_trace(True)
    ...
    with torch.no_grad():
        ...
        def reconstruct(model, dataset, device="cpu"):
            ...
            for data in dataset:
                ...
                # n_aug = 0
                # x_aug = torch.cat(
                #     [x_aligned, torch.zeros(x.shape[0], n_aug).to(x.device)], -1
                # )
                z0, *_ = super(
                    RegularisedContinuousManifoldFlow, model.pdm.layers.manifold_flow
                ).inverse(x=x_aligned, batch=None, condition=condition)
                #z0 = z0[:, :-n_aug]
                z1 = model.pdm.layers.manifold_flow.chart.inverse(z0)

                z2, *_ = model.pdm.layers.ambient_flow.inverse(
                    x=z1, batch=None, condition=condition
                )

                # forward
                x0, *_ = model.pdm.layers.ambient_flow.forward(
                    z=z2, batch=None, condition=condition
                )
                x1 = model.pdm.layers.manifold_flow.chart.forward(x0)

                # x1_aug = torch.cat(
                #     [x1, torch.zeros(x1.shape[0], n_aug).to(x.device)], -1
                # )
                x2, *_ = super(
                    RegularisedContinuousManifoldFlow, model.pdm.layers.manifold_flow
                ).forward(z=x1, batch=None, condition=condition)
        ...
        reconstruct(model, train, device=device)
